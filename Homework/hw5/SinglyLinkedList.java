...
public class SinglyLinkedList<AnyType extends Comparable<AnyType>> implements List<AnyType> {
    ...
    public void swap( int i, int j )  throws IndexOutOfBoundsException {
         ...
         * 
         */
        //rearrange data getData and setData methods
        if((i<0||i>=size)&&(j<0||j>=size))
            throw new IndexOutOfBoundsException();
        if(i!=j){
            AnyType nodeOneData = getNode(i).getData();
            AnyType nodeTwoData= getNode(j).getData();
            getNode(j).setData(nodeOneData);
            getNode(i).setData(nodeTwoData);
    ...
    public static void main( String[] args ) {
        ...
        System.out.println("write before next sort with random numbers");
        for(int i=0;i<12;i++) {
